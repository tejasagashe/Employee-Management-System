...
public class AddEmployee extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == add) {
            ...
            String Last_Name = tflname.getText();
            ...
            if(First_Name.length()==0 || Last_Name.length()==0 || DOB.length()==0 || Salary.length()==0 || Address.length()==0 || Phone.length()==0 || Email.length()==0 || Education.length()==0 || Designation.length()==0 || Aadhar.length()==0){
            JOptionPane.showMessageDialog(null, "Fill Details Correctly");
            }
            else{
                try {
                Conn conn = new Conn();
                String query = "insert into employee values('"+First_Name+"', '"+Last_Name+"', '"+DOB+"', '"+Salary+"', '"+Address+"', '"+Phone+"', '"+Email+"', '"+Education+"', '"+Designation+"', '"+Aadhar+"', '"+Employee_ID+"')";
